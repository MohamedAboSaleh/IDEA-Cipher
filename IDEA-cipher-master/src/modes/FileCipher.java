...
public class FileCipher extends Task<Void> {
	...
	// LoggerFactory.getLogger(FileCipher.class);
	private static final int BLOCK_SIZE = 8;
	public static List<BigInteger> encryption;
	private List<BigInteger> decryptedKey;
	...
	private void cryptFile() throws IOException {
		...
		try (FileChannel inChannel = FileChannel.open(Paths.get(input), StandardOpenOption.READ);
			...
			if (encrypt) {
				opMod = new OFB(key);
				encryption = IdeaCipher.rsa.encryptMessage(key);
				System.out.println("encripted(decimal)    = " + Utils.bigIntegerSum(encryption));
				inDataLen = inFileSize; // Input data size = input file size
				outDataLen = (inDataLen + BLOCK_SIZE - 1) / BLOCK_SIZE * BLOCK_SIZE; // Closest upper multiple of
																						// blockSize
				// logger.debug("Sizes: " + inDataLen + "b input, " + (outDataLen + BLOCK_SIZE)
				// + "b output");
				status.setValue("Input size: " + inDataLen / 1024 + "KB.");
			} else {
				decryptedKey = IdeaCipher.rsa.decrypt(encryption);
				System.out.println("decrypted(decimal)    = " + Utils.bigIntegerToString(decryptedKey));
				opMod = new OFB(Utils.bigIntegerToString(decryptedKey));
				if (inFileSize == 0) {
				...
				// logger.debug("Sizes: " + (inDataLen + BLOCK_SIZE) + "b input, <=" +
				// outDataLen + "b output");
	...
	/**
	 * Read the input file in chunks of 2MB, encrypt/decrypt the chunks and write it
	 * in the output file.
	 */
	private void processData(FileChannel inChannel, long inDataLen, FileChannel outChannel, long outDataLen,
		...
		while (filePos < inDataLen) {
			...
			// Encrypt chunk
			int chunkLen = (bytesRead + BLOCK_SIZE - 1) / BLOCK_SIZE * BLOCK_SIZE; // Closest upper multiple of
																					// blockSize
	...
	/**
	 * Write the length of the encrypted data in an encrypted block at the end of
	 * the file. The length is package is a 8-byte block, this block is encrypted
	 * and finally added at the end of output file.
	 */
	private void writeDataLength(FileChannel outChannel, long dataLength, OperationMode opMod) throws IOException {
	...
	/**
	 * Get the length of the data that was encrypted. This data is saved encrypted
	 * in the last block of the cryptogram. Read the last block of the file, decrypt
	 * block and unpackage data lenght.
	 */
	...
	private static long unpackDataLength(byte[] b) {
		...
		return (long) (b[7] & 0xFF) >> 3 | (long) (b[6] & 0xFF) << 5 | (long) (b[5] & 0xFF) << 13
				| (long) (b[4] & 0xFF) << 21 | (long) (b[3] & 0xFF) << 29 | (long) (b[2] & 0xFF) << 37;
	}
